...
/**
 * DTO para representar una línea de albarán en la tabla del formulario
 */
public class AlbaranLineaDTO {
    ...
    public AlbaranLineaDTO() {
        ...
        descuento.addListener((obs, oldVal, newVal) -> calcularTotales());
        iva.addListener((obs, oldVal, newVal) -> calcularTotales());
    }

    public AlbaranLineaDTO(Articulo articulo) {
        this();
    ...
    private void calcularTotales() {
        ...
        BigDecimal desc = descuento.get() != null ? descuento.get() : BigDecimal.ZERO;
        BigDecimal ivaVal = iva.get() != null ? iva.get() : BigDecimal.ZERO;

        // Subtotal = (cantidad * precio) - descuento
        BigDecimal sub = cant.multiply(prec).subtract(desc).setScale(2, RoundingMode.HALF_UP);
        if (sub.compareTo(BigDecimal.ZERO) < 0) sub = BigDecimal.ZERO;
        subtotal.set(sub);
