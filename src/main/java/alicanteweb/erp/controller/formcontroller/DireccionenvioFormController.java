...
public class DireccionenvioFormController {
    ...
    public void onGuardar() {
        try {
            ...
            DialogUtils.showInfo("Guardado");
            cerrar();
        } catch (Exception e) { log.error("Error guardando dirección", e); DialogUtils.showError(e.getMessage()); }
    }
    ...
    private boolean validar() {
        if (txtNombre == null || txtNombre.getText() == null || txtNombre.getText().trim().isEmpty()) { DialogUtils.showError("El nombre es obligatorio"); return false; }
        return true;
    }
